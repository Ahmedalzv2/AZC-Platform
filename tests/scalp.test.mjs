...
describe('scalpMonitorTick', () => {
  ...
  test('happy path + live → signs and POSTs through Worker', async () => {
    ...
    // SILVER defaults to 200× (trio) → mechanical SL/TP, fee-aware TP.
    // entry 75.65 SHORT
    // SL: 0.4 × (100/200)% = 0.20% price → 75.65 × 1.0020 ≈ 75.80
    // TP: 10% net + 16% fee = 26% gross margin → 0.13% price →
    //     75.65 × (1 - 0.0013) ≈ 75.55
    assert.equal(body.price, 75.65);
    assert.ok(Math.abs(body.stopLossPrice - 75.80) < 0.02, `sl ${body.stopLossPrice}`);
